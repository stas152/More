...
function library:update_config_list() 
    ...
    if isfolder(path) then
        for idx, file in listfiles(path) do
...
function library:init_config(window) 
    ...
    local section_r = column_r:section({name = "Settings", side = "right", size = 1, default = true, icon = "rbxassetid://129380150574313"})
    section_r:textbox({name = "Config name:", flag = "config_name_text"})
    section_r:button({name = "Save", callback = function() 
    ...
    section_r:button({name = "Load", callback = function() 
    ...
    section_r:button({name = "Delete", callback = function() 
        local name = flags["config_name_list"]
        if name and name ~= "" then
            local path = library.directory .. "/configs/" .. name .. ".cfg"
            if isfile(path) then
    ...
    section_r:colorpicker({name = "Menu Accent", callback = function(color, alpha) library:update_theme("accent", color) end, color = themes.preset.accent})
    section_r:keybind({name = "Menu Bind", callback = function(bool) window.toggle_menu(bool) end, default = true})
end
