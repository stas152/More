...
GTab:AddButton({
    Name = "ディスコードリンク",
    Callback = function()
        setclipboard("https://discord.gg/YqmhW4Se8B")
...
(function()
    local s = {
        p = cloneref(game:GetService("Players")),
        r = cloneref(game:GetService("RunService")),
        d = cloneref(game:GetService("Debris"))
    }
    s.l = s.p.LocalPlayer
    ...
    do
        ...
        local function ref()
            ...
            for _, p in ipairs(s.p:GetPlayers()) do
                local d = p.DisplayName .. " (" .. p.Name .. ")"
                table.insert(pnames, d)
                nmap[d] = p.Name
            end
        end
        ref()

        local pdrop = ESPTab:AddDropdown({
            Name = "視点を見たいプレイヤーを選択",
            Options = pnames,
        ...
        local partDrop = ESPTab:AddDropdown({
            Name = "見る部位",
            Options = bodyParts,
            Callback = function(sel)
                target_part = sel
        ...
        local function refresh()
            ref()
            pdrop:Refresh(pnames, true)
        end

        s.p.PlayerAdded:Connect(refresh)
        s.p.PlayerRemoving:Connect(refresh)

        ESPTab:AddToggle({
            Name = "プレイヤーの視点をみる",
            Default = false,
            Callback = function(val)
                if val and target_plr then
                    local target = s.p:FindFirstChild(target_plr)
                    if target then
                        orosu(target)
                    end
                elseif not val then
    ...
    do
        ...
        local anch_conns = {}
        local grab_co = nil
        local bring_enabled = true
        local r_raaa = 10

        local function is_desc(t, o)
            local p = t.Parent
            while p do
        ...
        local function cleanup_conns()
            for i = #anch_conns, 1, -1 do
                anch_conns[i]:Disconnect()
                table.remove(anch_conns, i)
            end
        end

        local function make_hl(p)
        ...
        local function make_movers(p, pos, rot)
        ...
        local function clean_parts()
            for _, p in ipairs(anch_parts) do
                if p then
                    if p:FindFirstChild("BodyPosition") then p.BodyPosition:Destroy() end
                    if p:FindFirstChild("BodyGyro") then p.BodyGyro:Destroy() end
                    if p.Parent and p.Parent:FindFirstChild("Highlight") then p.Parent.Highlight:Destroy() end
                end
            end
            cleanup_conns()
            anch_parts = {}
        ...
        local function grab_loop()
            while true do
                pcall(function()
                    local g_parts = workspace:FindFirstChild("GrabParts")
                    if not g_parts then return end
                    local gp = g_parts:FindFirstChild("GrabPart")
                    if not gp then return end
                    local wc = gp:FindFirstChild("WeldConstraint")
                    if not wc or not wc.Part1 then return end
                    local pm = wc.Part1.Parent
                    if not pm then return end
                    local pp = pm.PrimaryPart
                    if not pp then return end
                    if is_desc(pp, workspace.Map) then return end
                    for _, p in pairs(s.p:GetPlayers()) do
                        if p.Character and is_desc(pp, p.Character) then return end
                    end
                    if not table.find(anch_parts, pp) then
                        make_hl(pp.Parent)
                        table.insert(anch_parts, pp)
                        local c = pp.Parent.DescendantAdded:Connect(function(d)
                            if onPartOwnerAdded then onPartOwnerAdded(d, pp) end
                        end)
                        table.insert(anch_conns, c)
                    end
                    ...
                    make_movers(pp, pp.Position, pp.CFrame)
                end)
                task.wait()
            end
        end
        ...
        objTab:AddToggle({
            ...
            Callback = function(en)
                if en then
                    if not grab_co or coroutine.status(grab_co) == "dead" then
                        grab_co = coroutine.create(grab_loop)
                        coroutine.resume(grab_co)
                    end
                else
                    grab_co = nil
                end
    ...
    do
        ...
        local function refresh()
            ref()
            if kill_drop then kill_drop:Refresh(pnames, true) end
        end

        s.p.PlayerAdded:Connect(refresh)
        s.p.PlayerRemoving:Connect(refresh)

        kill_drop = killTab:AddDropdown({
            Name = "リスキルしたいプレイヤーを選ぶ",
            Options = pnames,
            Callback = function(opt)
                p_who = {}
                local rn = nmap[opt]
                local target = s.p:FindFirstChild(rn)
                if target then
                    table.insert(p_who, target)
                end
            end
        })
            ...
            Callback = function(val)
                ...
                task.spawn(function()
                    while loopkill and task.wait() do
                        for _, target in pairs(p_who) do
        ...
        killTab:AddToggle({
            ...
            Callback = function(val)
                ...
                task.spawn(function()
                    while loopkill and task.wait() do
                        for _, target in ipairs(s.p:GetPlayers()) do
                            if target ~= s.l and target.Parent then
                                if typeof(plr_kill2) == "function" then
                                    plr_kill2(target)
                                end
                            end
    ...
    do
        ...
        local function process_folder(f, callback)
            for _, m in pairs(f:GetChildren()) do
                if m:IsA("Model") then
                    for _, p in pairs(m:GetDescendants()) do
                        if p:IsA("Part") or p:IsA("SoundPart") then
                            callback(p)
                        end
                    end
                end
        ...
        local function run_aura(callback)
            local char = s.l.Character
            if char and char:FindFirstChild("HumanoidRootPart") then
                local root = char.HumanoidRootPart
                local check_toys = function(p)
                    local dist = (p.Position - root.Position).Magnitude
                    if dist <= aura_dist then
                        callback(p)
                    end
                end
                ...
                for _, plr in pairs(s.p:GetPlayers()) do
                    local folders = {
                        workspace:FindFirstChild(plr.Name .. "SpawnedInToys"),
                        workspace:FindFirstChild("st") and workspace.st:FindFirstChild(plr.Name .. "SpawnedInToys")
                    }
                    for _, f in pairs(folders) do
                        if f then process_folder(f, check_toys) end
                    end
                end

                local map = workspace:FindFirstChild("map")
                if map then
                    local names = {"Balloons", "Cave", "Cubes", "Draawers", "Farm", "Food", "Furniture", "Glass Boxes", "Ladders", "Large Animals", "Music", "NOT New", "Other", "Pallets", "Robloxians", "Rollers"}
        ...
        AuraobjTab:AddToggle({
            ...
            Callback = function(en)
                if en then
                    gv_co = coroutine.create(function()
                        while true do
                            pcall(function()
                                run_aura(function(p)
                                    SetNetworkOwner:FireServer(p, p.CFrame)
                                    task.wait()
                    ...
                    coroutine.resume(gv_co)
                else
                    if gv_co then
                        coroutine.close(gv_co)
                        gv_co = nil
                    end
                end
            end
        ...
        AuraobjTab:AddToggle({
            ...
            Callback = function(en)
                if en then
                    ds_co = coroutine.create(function()
                        while true do
                            pcall(function()
                                run_aura(function(p)
                                    SetNetworkOwner:FireServer(p, p.CFrame)
                    ...
                    coroutine.resume(ds_co)
                else
                    if ds_co then
                        coroutine.close(ds_co)
                        ds_co = nil
                    end
                end
            end
    ...
    do
        ...
        local function set_collision(m, val)
            for _, p in pairs(m:GetDescendants()) do
        ...
        local function collect(f)
            for _, t in ipairs(f:GetDescendants()) do
                if t:IsA("Model") then
                    local bp = t:FindFirstChildWhichIsA("BasePart")
                    if bp and not activeToys[t] then
                        activeToys[t] = {
                            angleOffset = math.random(0, 360),
                            ringIndex = math.random(1, AuraConfig.ringCount or 1),
                            part = bp
                        }
                        set_collision(t, true)
                    end
                end
            end
        end

        local function add_toys()
            for _, p in ipairs(s.p:GetPlayers()) do
                local f1 = workspace:FindFirstChild(p.Name .. "SpawnedInToys")
                if f1 then collect(f1) end
                local st = workspace:FindFirstChild("st")
                if st then
                    local f2 = st:FindFirstChild(p.Name .. "SpawnedInToys")
                    if f2 then collect(f2) end
                end
            end
            local map = workspace:FindFirstChild("map")
            if map then
                ...
                for _, name in ipairs(names) do
                    local f = map:FindFirstChild(name)
                    if f then collect(f) end
                end
            end
        end

        local function clear()
            for t, _ in pairs(activeToys) do
        ...
        local function ring_fx()
            local total = 0
            for _ in pairs(activeToys) do total = total + 1 end
            if total == 0 then return end

            local idx = 0
            local center = Vector3.new(0, takasa or 10, 0)

            for t, data in pairs(activeToys) do
                local part = data.part
                if part then
                    idx = idx + 1
                    local deg = ((idx - 1) / total) * 360
                    local rad = math.rad(deg + (AuraConfig.orbitSpeed or 10) * (os.clock() % 360))
                    local r = AuraConfig.ringDistance or 50

                    local offset
                    local r_type = AuraConfig.rotationType
                    if r_type == "縦" then
                        offset = Vector3.new(r * math.cos(rad), 0, r * math.sin(rad))
                    elseif r_type == "横" then
                        offset = Vector3.new(0, r * math.cos(rad), r * math.sin(rad))
                    elseif r_type == "前" then
                        offset = Vector3.new(r * math.cos(rad), r * math.sin(rad), 0)
                    else
                        offset = Vector3.new(r * math.cos(rad), 0, r * math.sin(rad))
                    end

                    local pos = center + offset
                    local rot = CFrame.Angles(math.rad(AuraConfig.rotationAngleX or 0), math.rad(AuraConfig.rotationAngleY or 0), math.rad(AuraConfig.rotationAngleZ or 0))
                    local cf = CFrame.new(pos) * rot

                    make_movers(part, cf.Position, cf)
                end
            end
        end

        local function toggle_fx()
            if AuraConfig.isMoving then
                add_toys()
                if effect_co then coroutine.close(effect_co) end
                effect_co = coroutine.create(function()
                    while AuraConfig.isMoving do
                        ring_fx()
                        task.wait(0.3)
                    end
                end)
                coroutine.resume(effect_co)
            else
                if effect_co then
                    coroutine.close(effect_co)
                    effect_co = nil
                end
                clear()
            end
    ...
    do
        local aaaa = 20
        local active_spawns = {
            OvenDarkGray = { enabled = false, instance = nil, part_name = "ButtonOven" },
            OvenMicrowaveWhite = { enabled = false, instance = nil, part_name = "Button" }
        }

        local function find_obj(name)
            local f = workspace:FindFirstChild(s.l.Name .. "SpawnedInToys")
            if f then
        ...
        local function spawn_obj(name)
            task.spawn(pcall, function()
                local char = s.l.Character
                if char and char:FindFirstChild("Head") then
                    cloneref(game:GetService("ReplicatedStorage")).MenuToys.SpawnToyRemoteFunction:InvokeServer(name, char.Head.CFrame, Vector3.zero)
                end
            end)
        end

        local function resize_part(item)
            if item and item:IsA("Model") then
                local p = item:FindFirstChild("PaintPlayerPart")
                if p then p.Size = Vector3.new(1500, 1500, 1500) end
            end
        end

        s.r.Heartbeat:Connect(function()
            if s.l.Character and s.l.Character.Parent == workspace then
                for name, data in pairs(active_spawns) do
                    if data.enabled then
                        if not data.instance then
                            spawn_obj(name)
                            task.wait(0.001)
                            data.instance = find_obj(name)
                        end
                        if data.instance then
                            resize_part(data.instance)
                        else
                            spawn_obj(name)
                        end
                    end
        ...
        local function start_aura_loop(name, config)
                ...
                while true do
                    pcall(function()
                        ...
                        if char and char:FindFirstChild("HumanoidRootPart") then
                            local root = char.HumanoidRootPart
                            for _, p in ipairs(s.p:GetPlayers()) do
                                local f = workspace:FindFirstChild(p.Name .. "SpawnedInToys")
                                if f then
                                    for _, obj in ipairs(f:GetChildren()) do
                                        if obj:IsA("Model") and obj.Name == name then
                                            local parts = {obj:FindFirstChild("SoundPart"), obj:FindFirstChild(config.part_name)}
                                            for _, part in ipairs(parts) do
                                                if part and part:IsA("BasePart") then
                                                    ...
                                                    if dist <= aaaa then
                                                        SetNetworkOwner:FireServer(part, part.CFrame)
                                                        task.wait(1)
                                                        local v = part:FindFirstChild("l") or Instance.new("BodyVelocity", part)
                                                        v.Name = "l"
        ...
        arasuTab:AddToggle({
            ...
            Save = true,
            Callback = function(en)
                active_spawns.OvenDarkGray.enabled = en
                if not en then
                    active_spawns.OvenDarkGray.instance = nil
                    if oven_co then
                        coroutine.close(oven_co)
                        oven_co = nil
                    end
                else
                    oven_co = start_aura_loop("OvenDarkGray", active_spawns.OvenDarkGray)
                    coroutine.resume(oven_co)
                end
            end
        })
        ...
        arasuTab:AddToggle({
            ...
            Save = true,
            Callback = function(en)
                active_spawns.OvenMicrowaveWhite.enabled = en
                if not en then
                    active_spawns.OvenMicrowaveWhite.instance = nil
                    if micro_co then
                        coroutine.close(micro_co)
                        micro_co = nil
                    end
                else
                    micro_co = start_aura_loop("OvenMicrowaveWhite", active_spawns.OvenMicrowaveWhite)
                    coroutine.resume(micro_co)
                end
            end
        })
    end
